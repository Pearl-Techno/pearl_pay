...
class _LoanRatesScreenState extends State<LoanRatesScreen> {
  ...
  // Load current Loan rate
  Future<void> _loadLoanRate() async {
    ...
    try {
     ...
     // final rate = await widget.apiService.getLoanRate(
     //     widget.user['company_id'].toString());
     // setState(() {
     //   _loanRateController.text = rate['loan_rate']?.toString() ?? '5.0';
     // });
      final prefs = await SharedPreferences.getInstance();
      final rate = prefs.getDouble('loan_rate') ?? 5.0;
      setState(() {
  ...
  // Save Loan rate
  Future<void> _saveLoanRate() async {
    ...
    try {
     ...
     // await widget.apiService.updateLoanRate(data);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Loan rate saved successfully'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error saving Loan rate: $e'),
          backgroundColor: Colors.red[700],
          action: SnackBarAction(
            label: 'Retry',
            textColor: Colors.white,
            onPressed: _saveLoanRate,
          ),
        ),
      );
    } finally {
                            ...
                            children: [
                              if (!_isAdmin)
                                Padding(
                                  ...
                                  child: Text(
                                    'Note: Only admins can edit Loan rates.',
                                    style: TextStyle(
                                      fontSize: 14,
                              ...
                              Form(
                                ...
                                child: TextFormField(
                                  ...
                                  validator: (value) {
                                    ...
                                    if (rate <= 0 || rate > 20) {
                                      return 'Rate must be between 0 and 20%';
                                    }
                                    return null;
                                  },
                                  readOnly: !_isAdmin,
                              ...
                              if (_isAdmin) ...[
                                ...
                                ElevatedButton(
                                  onPressed: _isLoading ? null : _saveLoanRate,
