...
class SelfAppraisalProvider with ChangeNotifier {
  ...
  void updateField<T>(String field, T value) {
    ...
    } else if (field == 'trainingAttended' || field == 'training') {
    ...
    try {
      ...
      await showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          title: Text('Success', style: GoogleFonts.roboto(fontSize: 20, fontWeight: FontWeight.w600, color: Colors.teal[800])),
          content: Text('Self-appraisal submitted successfully!', style: GoogleFonts.roboto(fontSize: 16, color: Colors.black87)),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
              child: Text('OK', style: GoogleFonts.roboto(color: Colors.teal)),
            ),
          ],
        ),
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error: $e', style: GoogleFonts.roboto(color: Colors.white)),
...
class _SelfAppraisalViewState extends State<_SelfAppraisalView> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar(
        title: 'Self Appraisal for $employeeName',
                  ...
                  child: Column(
                    ...
                    children: [
                      _buildSection(
                        ...
                        children: [
                          Text(
                            'Name: $employeeName (ID: ${widget.user.employeeId ?? 'N/A'})',
                            style: GoogleFonts.roboto(fontSize: 16, color: Colors.teal[800]),
                          ),
                          _buildTextField(
                            label: 'Designation',
                      ...
                      _buildSection(
                        ...
                        children: [
                          ...provider.improvementPlan.asMap().entries.map((entry) => _buildImprovementPlanEntry(
                                index: entry.key,
                          ...
                          TextButton.icon(
                            ...
                            label: const Text('Add Entry', style: TextStyle(color: Colors.teal)),
  ...
  Widget _buildSection({
    ...
    return Container(
      ...
      child: ExpansionTile(
        ...
        title: Text(
          title,
          style: GoogleFonts.roboto(fontSize: 18, fontWeight: FontWeight.w600, color: Colors.teal[800]),
        ),
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.teal[700]!, width: 2)),
          errorBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Colors.red)),
          suffixIcon: value.isNotEmpty ? const Icon(Icons.check_circle, color: Colors.green) : null,
        ),
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.teal[700]!, width: 2)),
          errorBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Colors.red)),
          suffixIcon: value > 0 ? const Icon(Icons.check_circle, color: Colors.green) : null,
        ),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            label,
            style: GoogleFonts.roboto(fontSize: 16, color: Colors.teal[800], fontWeight: FontWeight.w500),
          ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              label,
              style: GoogleFonts.roboto(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.teal[900]),
            ),
            const SizedBox(height: 12),
            _buildStarRatingField(label: 'Self Rating', value: selfRating, onChanged: onSelfRatingChanged),
            if (onDescriptionChanged != null)
              _buildTextField(label: 'Description', value: description ?? '', onChanged: onDescriptionChanged),
            if (onCommentsChanged != null)
              _buildTextField(label: 'Comments', value: comments ?? '', onChanged: onCommentsChanged),
          ],
        ),
      ),
    );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            _buildTextField(label: 'Area for Improvement', value: area, onChanged: onAreaChanged),
            _buildTextField(label: 'Action Plan', value: action, onChanged: onActionChanged),
            _buildTextField(label: 'Goal', value: goal, onChanged: onGoalChanged),
            _buildTextField(label: 'Timing', value: timing, onChanged: onTimingChanged),
          ],
        ),
      ),
    );
            ...
            child: ElevatedButton(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text('Preview', style: GoogleFonts.roboto(fontSize: 16, color: Colors.white)),
                ],
              ),
            ),
          ),
              ...
              child: provider.isSubmitting
                  ...
                  : Row(
                      ...
                      children: [
                        ...
                        const SizedBox(width: 8),
                        Text('Submit', style: GoogleFonts.roboto(fontSize: 16, color: Colors.white)),
                      ],
                    ),
            ),
          ),
            ...
            child: OutlinedButton(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text('Clear', style: GoogleFonts.roboto(fontSize: 16, color: Colors.red[700])),
                ],
              ),
            ),
          ),
    ...
    showDialog(
      ...
      builder: (context) => DefaultTabController(
        ...
        child: AlertDialog(
          ...
          title: Column(
            children: [
              Text('Self-Appraisal Preview', style: GoogleFonts.roboto(fontSize: 20, fontWeight: FontWeight.w600, color: Colors.teal[900])),
              const SizedBox(height: 8),
              Container(
                decoration: BoxDecoration(
            ...
            child: TabBarView(
              children: [
                _buildPreviewSection('Personal Details', [
                  _buildPreviewItem('Name', widget.user.employeeId ?? 'N/A'),
                  _buildPreviewItem('Designation', provider.designation),
                  _buildPreviewItem('Period Under Review', provider.periodUnderReview),
                ]),
                _buildPreviewSection('Performance Indicators', [
                  _buildPreviewItem('Awards', provider.awards),
                  _buildPreviewItem('Training Attended', provider.trainingAttended),
                  _buildPreviewItem('Valid Warnings', provider.numberValidWarnings.toString()),
                  _buildPreviewItem('Absentee Days', provider.absenteeDays.toString()),
                  _buildPreviewItem('Sick Days', provider.sickDays.toString()),
                ]),
                _buildPreviewSection('Personal Attributes', [
                  ...provider.personalAttributes.entries.map((entry) => [
                    _buildPreviewItem('${entry.key.replaceAll('_', ' ').toTitleCase()} (Self)', '${entry.value['self'].index} Stars'),
                    _buildPreviewItem('${entry.key.replaceAll('_', ' ').toTitleCase()} Comments', entry.value['comments']),
                  ]).expand((e) => e),
                ]),
                _buildPreviewSection('Operational Skills', [
                  ...provider.operationalSkills.entries.map((entry) => [
                    _buildPreviewItem('${entry.key.replaceAll('_', ' ').toTitleCase()} (Self)', '${entry.value['self'].index} Stars'),
                    if (entry.key.startsWith('essential_function'))
                      _buildPreviewItem('${entry.key.replaceAll('_', ' ').toTitleCase()} Description', entry.value['description']),
                    _buildPreviewItem('${entry.key.replaceAll('_', ' ').toTitleCase()} Comments', entry.value['comments']),
                  ]).expand((e) => e),
                ]),
                _buildPreviewSection('Compliance Questions', [
                  _buildPreviewItem('Compliance Training', provider.complianceTraining ? 'Yes' : 'No'),
                  _buildPreviewItem('Punctuality', provider.punctuality ? 'Yes' : 'No'),
                  _buildPreviewItem('Internal Controls', provider.internalControls ? 'Yes' : 'No'),
                  _buildPreviewItem('Professionalism', provider.professionalism ? 'Yes' : 'No'),
                ]),
                _buildPreviewSection('Improvement Plan', [
                  ...provider.improvementPlan.asMap().entries.map((entry) => [
                    _buildPreviewItem('Entry ${entry.key + 1}: Area', entry.value['area']!),
                    _buildPreviewItem('Entry ${entry.key + 1}: Action Plan', entry.value['action_plan']!),
                    _buildPreviewItem('Entry ${entry.key + 1}: Goal', entry.value['goal']!),
                    _buildPreviewItem('Entry ${entry.key + 1}: Timing', entry.value['timing']!),
                  ]).expand((e) => e),
                ]),
                _buildPreviewSection('Career Development', [
                  _buildPreviewItem('Career Objectives', provider.careerObjectives),
                  _buildPreviewItem('Long-Term Development Plan', provider.longTermDevelopmentPlan),
                  _buildPreviewItem('Job Targets', provider.jobTargets),
                  _buildPreviewItem('Development Needs', provider.developmentNeeds),
                  _buildPreviewItem('Training Required', provider.trainingRequired),
                ]),
                _buildPreviewSection('Summary', [
                  _buildPreviewItem('Overall Self Rating', '${provider.overallRatingSelf.index} Stars'),
                  _buildPreviewItem('Comments', provider.appraiseeComments),
                ]),
              ],
            ),
          ),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
              child: Text('Cancel', style: GoogleFonts.roboto(color: Colors.grey[600])),
            ),
            ElevatedButton(
              onPressed: () {
                Navigator.pop(context);
...
// Helper widget and extension moved outside the class
    ...
    child: Column(
      ...
      children: [
        Padding(
          ...
          child: Text(
            title,
            style: GoogleFonts.roboto(fontSize: 18, color: Colors.teal[800], fontWeight: FontWeight.w600),
          ),
        ),
        ...items,
        const Divider(),
    ...
    child: Row(
      ...
      children: [
        Expanded(
          ...
          child: Text(
            '$label:',
            style: GoogleFonts.roboto(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.teal[700]),
          ),
        ),
        Expanded(
          flex: 3,
